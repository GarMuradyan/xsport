function renderRegisterPage() {
    let registerPageBox = el('div','register_page-box')

    let registerRowBox = el('div','register_row-box')

    let rowLogoBox = el('div','row_logo-box')

    let rowLogo = el('img','row-logo')
    rowLogo.src = 'data:image/png;base64,'
    
    let rowTitleBox = el('h3','row_title-box')
    rowTitleBox.textContent = 'Registration'
    
    rowLogoBox.append(rowLogo)
    
    registerRowBox.append(rowLogoBox)
    registerRowBox.append(rowTitleBox)
    registerRowBox.append(renderRowBottomBox())

    registerPageBox.append(registerRowBox)
    
    return registerPageBox
}

function renderRowBottomBox() {
    let rowBottomBox = el('div','row_bottom-box')

    let rowBottomEmailInputBox = el('div','row_bottom_email-input-box')

    let rowBottomEmailInput = el('input','row_bottom_email-input')
    rowBottomEmailInput.setAttribute('placeholder','Email addres')

    let rowBottomFullNameInputBox = el('div','row_bottom_full-name-input-box')

    let rowBottomFullNameInput = el('input','row_bottom_full-name-input')
    rowBottomFullNameInput.setAttribute('placeholder','Enter your full name')

    let errOrResMessage = el('p','err-or-res-message')

    let rowBottomButtonBox = el('button','row_bottom_button-box')
    rowBottomButtonBox.textContent = 'Next'
    rowBottomButtonBox.onclick = () => {
        renderRegister()
    }

    let rowBottomLoginBox = el('div','row_bottom_login-box')

    let rowBottomLogin = el('span','row_bottom_login')
    rowBottomLogin.textContent = 'Login'
    rowBottomLogin.onclick = function () {
        window.location.href = "../index.html"
    }

    rowBottomLoginBox.append(rowBottomLogin)

    rowBottomEmailInputBox.append(rowBottomEmailInput)

    rowBottomFullNameInputBox.append(rowBottomFullNameInput)

    rowBottomBox.append(rowBottomEmailInputBox)
    rowBottomBox.append(rowBottomFullNameInputBox)
    rowBottomBox.append(errOrResMessage)
    rowBottomBox.append(rowBottomButtonBox)
    rowBottomBox.append(rowBottomLoginBox)

    return rowBottomBox
}

function renderRegister() {
    const regHost = 'https://x-sport-api.inorain.com'

    const regRout = '/users/auth/register/link'
    let rowBottomEmailInput = document.querySelector('.row_bottom_email-input')
    let rowBottomFullNameInput = document.querySelector('.row_bottom_full-name-input')
    let errOrResMessage = document.querySelector('.err-or-res-message')
    var validRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;

    let data = {
        email: rowBottomEmailInput.value,
        username: rowBottomFullNameInput.value
    }

    let header = {

    }

    let params = {}

    if (rowBottomEmailInput.value.match(validRegex)) {
        req('post',regHost+regRout,data,header,params,(res) => {
            console.log(res);
            errOrResMessage.textContent = res.data.message
            errOrResMessage.style.color = 'green'
        },(err)=> {
            console.log(err);
            errOrResMessage.textContent = err.response.data.message
            errOrResMessage.style.color = 'red'
        })
    }else {
        errOrResMessage.textContent = 'invalid email'
        errOrResMessage.style.color = 'red'
        console.log('err');
        setTimeout(() => {
            errOrResMessage.textContent = ''
        }, 4000);
    }
}